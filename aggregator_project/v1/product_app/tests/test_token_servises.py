...
def test_token_existence_verification(to_create: bool, expected_result: bool):
    if to_create:
...
@responses.activate
@pytest.mark.django_db
@pytest.mark.parametrize("status", [300, 400, 500])
def test_load_access_token_from_offers_microservice_with_diff_wrong_status(status: int):
    responses.post(
        settings.BASE_OFFER_MICROSERVICE_API + settings.MICROSERVICE_AUTH_PATH, json={"msg": "error"}, status=status
    )
