...
def test_get_product_offers_success():
    responses.get(
        settings.BASE_OFFER_MICROSERVICE_API
        + f"/products/{MOCKED_PRODUCT_ID}"
...
def test_get_product_offers_failed_connection_exception():
    with mock.patch(
        "v1.product_app.services.offer_services.get_product_offers", side_effect=requests.exceptions.ConnectionError
    ):
...
@responses.activate
@pytest.mark.django_db
def test_create_or_update_product_offers_success(product_factory: ProductFactory):
...
@responses.activate
@pytest.mark.django_db
def test_create_or_update_product_two_load_product_offers(product_factory: ProductFactory):
    product = product_factory.create(id=MOCKED_PRODUCT_ID)
    responses.get(
        settings.BASE_OFFER_MICROSERVICE_API
        + f"/products/{MOCKED_PRODUCT_ID}"
...
@responses.activate
@pytest.mark.django_db
def test_create_or_update_product_offers_failed(product_factory: ProductFactory):
